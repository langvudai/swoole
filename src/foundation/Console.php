...
class Console 
{
    ...
    final public function __construct(string $base_path = null, string $application_namespace = '', string $application_path = '')
    {
        ...
        $this->server_settings = ['http_host' => '0.0.0.0', 'http_port' => 9501];
    ...
    {
        return $this->server_settings[$name] ?? null;
    }

    public function httpHost(string $hots, int $port = null)
    {
        $this->server_settings['http_host'] = $hots;

        if ($port) {
            $this->server_settings['http_port'] = $port;
        }
    }
     ...
     * @return void
     */
    public function withServerSentEvent(string $host = '0.0.0.0', int $port = 9505): void
    {
        $this->server_settings['sse_host'] = $host;
        $this->server_settings['sse_port'] = $port;
    }
     ...
     * @return void
     */
    final public function swooleServer($server, callable $callback = null)
    {
        if (!$this->server) {
            $this->server = $server;
        }
     ...
     * @return void
     */
    public function clientPost(array $data, string $host, int $port, callable $callback = null, array $headers = [])
    {
        if (is_callable($this->client_post)) {
            call_user_func_array($this->client_post, [$data, $host, $port, $callback, $headers]);
        }
    }
    ...
    final public function dispatch($http_request): mixed
    {
        ...
        try {
            ...
            $terminal_middlewares = [];

            if ($this->middlewares && is_array($this->middlewares)) {
                foreach ($this->middlewares as $middleware) {
                    $terminal_middlewares[] = $di->middleware($middleware);
                }
            }
            ...
            $terminal_middlewares = array_filter($terminal_middlewares);

            if ($terminal_middlewares) {
                foreach (array_reverse($terminal_middlewares) as $middlewares) {
                    if (method_exists($middlewares, 'terminal')) {
                        $ref = new ReflectionMethod($middlewares, 'terminal');

                        if (0 === $ref->getNumberOfParameters()) {
                            $ref->invoke($middlewares);
                        }
                    }
                }
            }
